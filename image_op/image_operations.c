...
Tree surface_to_matrix(SDL_Surface *surface)
{
...
Tree  y_cut(Tree node, int level)
{
  ...
  else
  {
    ...
    for(size_t j = 0; j < (cols - y); j++)
    {
      for(size_t i = 0; i < lines; i++)
      {
        upper[i * (cols - y)  + j] = pixel_matrix[i * (cols - y) + j];
      }
    }
    nodel->key = upper;
...
Tree x_cut(Tree node, int level)
{
  ...
  else
  {
    ...
    for(size_t i = 0; i < x; i++)
    {
      ...
      {
        upper[ i * cols + j] = pixel_matrix[i * cols + j];
      }
    }
    for(size_t i = 0; i < (lines - x); i++)
    {
      for(size_t j = 0; j < cols; j++)
      {
        upper[i * (lines - x)  + j] = pixel_matrix[i * (lines - x) + j];
      }
    }
    nodel->key = left;
    nodel->key_lines = x;
